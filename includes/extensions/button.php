		...
		$standard_layout = array(
			...
			'params' => array(
				...
				array(
					...
					'label' => __('Button tag', 'fusion-extension-button'),
					'help' => __('Use <button> tag output instead of <a> tag.' , 'fusion-extension-button')
				)
			)
		);
...
function fsn_get_standard_button($atts = false, $content = false) {
	...
	if (!empty($button)) {
		...
		$button_tag_output = 'a';
		if(!empty($button_tag)) {
			$button_tag_output = 'button';
		}

		$output .= apply_filters('fsn_standard_button_output', '<' . $button_tag_output . fsn_get_button_anchor_attributes($button_object, $button_classes) . '>' . esc_html($button_object['button_label']) . '</' . $button_tag_output . '>', $atts);
	}
